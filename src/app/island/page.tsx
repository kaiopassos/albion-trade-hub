    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-[#e8d5b5]">Ilha</h2>
            <p className="text-[#8b7635] text-sm">Planeje sua ilha e maximize o retorno.</p>
          </div>
        </div>
        <button onClick={handleSave} className="rounded-lg bg-[#c8a84e] px-4 py-2 text-sm font-medium text-[#1a1410] hover:bg-[#d4b85e] transition-colors">
          {saved ? "Salvo!" : "Salvar"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-lg border border-[#3a3028] bg-[#241e18] p-4 space-y-2">
          ...
          <select value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)}
            className="w-full rounded-lg border border-[#3a3028] bg-[#12100c] px-3 py-2 text-sm text-[#e8d5b5]">
      ...
      {cityBonus && (
        <div className="rounded-lg border border-[#c8a84e]/20 bg-[#c8a84e]/5 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#a89070] mt-1">{cityBonus.description}</p>
      ...
      {/* Tabs */}
      <div className="flex gap-2 border-b border-[#3a3028] pb-2">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 rounded-t-lg px-4 py-2 text-sm font-medium transition-colors ${
              activeTab === tab.id ? "bg-[#241e18] text-[#c8a84e] border border-[#3a3028] border-b-transparent" : "text-[#8b7635] hover:text-[#a89070]"
            }`}>
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-medium text-[#e8d5b5]">Plantacoes</h3>
            </div>
            <div className="rounded-lg border border-[#3a3028] overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#3a3028]">
                  {FARM_CROPS.map(crop => (
                    <tr key={crop.id} className="hover:bg-[#2e2620]">
                      <td className="px-4 py-3 font-medium text-[#e8d5b5]">{crop.name}</td>
                      <td className="px-4 py-3 text-center text-[#a89070]">T{crop.tier}</td>
                      <td className="px-4 py-3 text-right font-mono text-[#a89070]">{formatSilver(crop.seedCost)}</td>
                      <td className="px-4 py-3 text-center text-[#a89070]">{crop.growthTime}</td>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-medium text-[#e8d5b5]">Criacoes</h3>
            </div>
            <div className="rounded-lg border border-[#3a3028] overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-[#12100c]">
                ...
                <tbody className="divide-y divide-[#3a3028]">
                  {FARM_ANIMALS.map(animal => (
                    <tr key={animal.id} className="hover:bg-[#2e2620]">
                      <td className="px-4 py-3 font-medium text-[#e8d5b5]">{animal.name}</td>
                      <td className="px-4 py-3 text-center text-[#a89070]">T{animal.tier}</td>
                      <td className="px-4 py-3 text-right font-mono text-[#a89070]">{formatSilver(animal.seedCost)}</td>
                      <td className="px-4 py-3 text-center text-[#a89070]">{animal.growthTime}</td>
      ...
      {activeTab === "guide" && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-sm font-medium text-[#e8d5b5]">Recomendacoes para voce</h3>
            <span className="text-xs text-[#8b7635]">(Tier {islandTier} em {selectedCity}, {formatSilver(silverBudget)} silver)</span>
          </div>
          {recommendations.map((tip, i) => (
            <div key={i} className="rounded-lg border border-[#3a3028] bg-[#241e18] p-4 flex items-start gap-3">
              <span className="rounded-full bg-[#c8a84e]/10 px-2 py-0.5 text-xs text-[#c8a84e] font-bold shrink-0">{i + 1}</span>
              <p className="text-sm text-[#a89070]">{tip}</p>
            </div>
          ))}

          <div className="mt-6">
            <h3 className="text-sm font-medium text-[#e8d5b5] mb-3">Comparativo de Cidades</h3>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {CITY_BONUSES.map(city => (
                <div key={city.city} className={`rounded-lg border p-4 space-y-2 transition-colors ${
                  city.city === selectedCity ? "border-[#c8a84e]/40 bg-[#c8a84e]/5" : "border-[#3a3028] bg-[#241e18]"
                }`}>
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-sm text-[#e8d5b5]">{city.city}</p>
                    {city.city === selectedCity && <span className="text-[10px] bg-[#c8a84e] text-[#1a1410] px-2 py-0.5 rounded font-bold">SUA ILHA</span>}
                  </div>
